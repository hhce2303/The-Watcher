...
//
// Public API: none — the view is self-contained. Replace the mock
// `notification` property with a real binding when connecting Python.
...
Item {
    ...
    // True when there is a request to work on. Bind `notification` to
    // backend.currentRequest in production; it is null when no requests exist.
    readonly property bool hasRequest: notification !== null && notification !== undefined

    // Mock notification (1 at a time, no queue)
    property var notification: ({
        ...
        payload: {
            ...
            description: "Auth error en cluster prod-api durante sesión activa. Recortar desde el primer 401 hasta el reconnect manual del operador. Incluir 5s de pre-roll si es posible."
        },
    ...
    ColumnLayout {
        ...
        Rectangle {
            ...
            RowLayout {
                ...
                Text { text: "THE WATCHER"
                       ...
                       font.pixelSize: 12; font.weight: Font.Bold; font.letterSpacing: 2.0 }
                Text { text: "v0.8.2"
                ...
                Rectangle {
                    ...
                    Layout.preferredWidth: demoRow.implicitWidth + 18
                    radius: W.Tokens.rXs
                    color: dh.hovered ? W.Tokens.bgElevated : W.Tokens.bgSurface
                    border.color: W.Tokens.borderBase; border.width: 1
                    HoverHandler { id: dh }
                    TapHandler   { onTapped: root.phase =
                                   (root.phase === "incoming"
                                    ? "editing" : "incoming") }
                    RowLayout {
                        id: demoRow
                        ...
                        spacing: 6
                        Text { text: "🔔"; color: W.Tokens.textMuted; font.pixelSize: 10 }
                        Text {
                            text: "DEMO · " + (root.phase === "incoming"
                                               ? "EDITANDO" : "NUEVA SOLICITUD")
                            color: W.Tokens.textMuted
                            font.family: W.Tokens.mono
        ...
        Item {
            ...
            W.NotificationStrip {
                ...
                onAccepted: root.phase = "editing"
                onDeclined: { /* would re-queue in production */ }
        ...
        Rectangle {
                ...
                ColumnLayout {
                    ...
                    Rectangle {
                        ...
                        Text { anchors.centerIn: parent; text: "✓"
                               color: W.Tokens.accentOk; font.pixelSize: 24 }
                    }
            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    Rectangle {
                        ...
                        Layout.alignment: Qt.AlignHCenter
                        Text { anchors.centerIn: parent; text: "🔔"
                               color: W.Tokens.accentPrimary; font.pixelSize: 24 }
                    }
            ...
            RowLayout {
                ...
                W.OutputPanel {
                    ...
                    onSaveRequested: root.saveState = "uploading"
        ...
        Rectangle {
            ...
            RowLayout {
                ...
                Rectangle { Layout.preferredWidth: 1; Layout.preferredHeight: 12
                            color: W.Tokens.borderBase }
                Text { text: root.hasRequest ? "1 tarea activa" : "Sin tareas"
                       color: W.Tokens.textMuted
