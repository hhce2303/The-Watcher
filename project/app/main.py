...
from app.core.role import OPERATOR, SUPERVISOR, enforce_role
...
def main() -> None:
    _instance_lock = _acquire_single_instance_lock()  # noqa: F841 — must stay in scope
    ...
    # Supervisor PCs don't record — skip the "no monitors" fatal check.
    if not all_monitors and user_config.role != SUPERVISOR:
        logger.critical("No monitors detected — cannot start recording.")
        sys.exit(1)
    elif not all_monitors:
        logger.info("Supervisor role — no monitors required, skipping recording setup.")
    ...
    if user_config.role != SUPERVISOR:
        ...
        logger.info("Supervisor role — recording stack not started.")
    ...
    # ── Start recording ───────────────────────────────────────────────
    if recording_service is not None and user_config.autorecord:
        recording_service.start()
        tray_module.set_recording_active(True)
    elif recording_service is not None:
        logger.info("Auto-record disabled in settings — buffer not started at launch.")
